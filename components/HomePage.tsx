	...
	const filteredMaterials = useMemo(() => {
		...
		if (!rawSearch) {
			return materials.filter(
				(m) =>
					selectedCategory === 'Todos' || m.type === selectedCategory
			);
		}
				...
				if (term.length > 3) {
					return materialWords.some((word) => {
						...
						if (word.length >= term.length) {
							...
							const distPrefix = getLevenshteinDistance(
								term,
								prefix
							);
			...
			<main className="flex-grow container mx-auto px-6 py-12">
				<section className="text-center mb-12">
					...
					<p className="text-lg text-gray-600 max-w-3xl mx-auto mb-8">
						Encontre apostilas, jogos e outros materiais educativos
						desenvolvidos pelo projeto TEDI para a terceira idade.
					</p>
				...
				<section id="materiais" className="scroll-mt-28">
					<div className="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
						<h2 className="text-3xl font-bold text-tedi-dark">
							Nossos Materiais
						</h2>
				...
				<section
					id="sobre"
					className="mt-20 bg-white p-8 md:p-12 rounded-xl shadow-md scroll-mt-28 border border-gray-100"
				>
					<div className="max-w-4xl mx-auto text-center">
						<h2 className="text-3xl font-bold text-tedi-dark mb-6">
							Sobre o Repositório GERAS
						</h2>
						<p className="text-gray-600 leading-relaxed text-lg text-justify md:text-center">
							...
							materiais criados pelo Projeto TEDI (Tecnologia e
							Educação para a Terceira Idade) da UTFPR. Nosso
							objetivo é oferecer um local único onde idosos,
							voluntários e a comunidade em geral possam
							encontrar, utilizar e compartilhar conhecimento,
							promovendo a inclusão digital e a autonomia da
							terceira idade.
						</p>
					</div>
				</section>
